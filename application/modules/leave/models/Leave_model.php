...
public function holiday_delete($id = null){
	...
	//new update 
	public function employee_annual_leaves(){

		$query=$this->db->query("(select *,'every year ' as type from employee_history  where employee_history.original_hire_date+INTERVAL 5 YEAR<=NOW() and 
		employee_history.employee_id NOT IN (select employee_annual_leave.Emp_ID  from employee_annual_leave  where employee_annual_leave.Start_Date+INTERVAL 1 YEAR<=NOW()) )       
		UNION
		(select *,' In 2 year ' as type  from employee_history  where employee_history.original_hire_date+INTERVAL 5 YEAR>=NOW() and 
		employee_history.employee_id NOT IN (select employee_annual_leave.Emp_ID  from employee_annual_leave  where employee_annual_leave.Start_Date+INTERVAL 2 YEAR<=NOW()) and employee_history.original_hire_date+INTERVAL 2 YEAR<=NOW()) ");
