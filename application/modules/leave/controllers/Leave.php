...
public function application(){ 
 ...
 // delete leave type
  ...
  public function delete_application($id = null){ 
		...
		if ($this->form_validation->run() === true) {
			...
			if ($this->Leave_model->update_application($postData)) { 
			  if($this->input->post('status',true) == 1){
			    $icon='http://palmtrix.com/HRM.png';
                		    $fields3 = array(
	...
	// new update annual leave history
	

	public function annual_leave_history(){   

        $this->permission->method('leave','read')->redirect();

		$data['title']  = display('annual_leave_list');  ;
		$data['leaves']   = $this->Leave_model->employee_annual_leaves();
		$data['module'] = "leave";
		$data['page']   = "annual_leave_history";   
		echo Modules::run('template/layout', $data); 
	} 
}
