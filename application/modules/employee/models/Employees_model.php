...
//new update
	...
	function emp_doc_search($id)
    {
        ...
        return $query->result();
			...
			echo 'Sorry No Document Found';
		}
    }

	public function emp_doc_create($data = array())
    {
        return $this->db->insert('employee_documents', $data);
    }

	
	public function deactivate_existing_docs($emp,$doctype)
    {
		$query=$this->db->query("UPDATE employee_documents SET IsActive=0 WHERE Emp_ID='".$emp."' and Doc_Name='".$doctype."' ");
	return 1;  
	}



	public function Expirydata_redzone()
	{
		
		$query=$this->db->query("SELECT * FROM `employee_documents` WHERE `Epx_Date`<NOW()+INTERVAL 1 MONTH and `IsActive`=1 and NOW()<`Epx_Date` ");

        return $data = $query->result();
	}
	public function Expirydata_orengezone()
	{
		
		$query=$this->db->query("select * FROM
		employee_documents
		where employee_documents.Epx_Date between NOW()+INTERVAL 1 month and NOW()+INTERVAL 2 month
		and `IsActive`=1 and NOW()<`Epx_Date` ");

        return $data = $query->result();
	}
	public function Expirydata_greenzone()
	{
		
		$query=$this->db->query("select * FROM
		employee_documents
		where employee_documents.Epx_Date between NOW()+INTERVAL 2 month and NOW()+INTERVAL 3 month
		and `IsActive`=1 and NOW()<`Epx_Date` ");
