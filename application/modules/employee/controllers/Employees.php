 ...
 */
 ...
 public function payslip(){   
		...
		public function update_emp_document(){  
		
			$this->permission->module('employee','read')->redirect();
	 
			$data['title']    = 'Emp Documents'; 
			$data['module']   = "employee";
			$data['page']     = "emp_document";   
			$data['dropdownemp']      = $this->Employees_model->Employeename();
			echo Modules::run('template/layout', $data); 
		} 
		
		public function search_emp_doc(){
			$id = $this->input->get('employee_id');
	
			$data['module']  = "employee";
			$data['emp_documents'] =  $this->Employees_model->emp_doc_search($id);
			$data['dropdownemp'] =$this->Employees_model->Employeename();
		...
		public function create_emp_doc()
		{ 
			$data['title'] = display('employee');
			$issue_date = $this->input->post('issue_date');
			$exp_date = $this->input->post('exp_date');
		
			$issue_time = date('Y-m-d H:i:s', strtotime($issue_date));
			$exp_time = date('Y-m-d H:i:s', strtotime($exp_date));
			// $id = $this->input->post('attendanc_id');
			#-------------------------------#intime
			$this->form_validation->set_rules('employee_id',display('employee_id'),'required');
			 $this->form_validation->set_rules('issue_date',display('issue_date'),'required');
			 $this->form_validation->set_rules('exp_date',display('exp_date'),'required');
						  //document upload
	...
	$filepath='./application/modules/employee/assets/images/'.$this->input->post('fileupld',true);
	...
	$docname=$othertext;
		...
		$docname=$this->input->post('doc_name',true);
	...
	//         #-------------------------------#
			if ($this->form_validation->run() === true) {
			  $emp_documnts = [
					'Emp_ID'    => $this->input->post('employee_id',true),
					'Doc_Name'  =>$docname,
					'Ref_ID'     => $this->input->post('reference_id',true),
					'Issuing_authority'   => $this->input->post('issue_auth',true),
				...
				$doctype=$docname;
			   //to deactivate already existing reocrd
					$docresult=$this->Employees_model->deactivate_existing_docs($empid,$doctype);
	
				if ($this->Employees_model->emp_doc_create($emp_documnts)) { 
			   ...
			   redirect("employee/Employees/update_emp_document");
		...
		public function view_emp_document_expiry()
		{ 
			$data['title'] = display('employee');
				...
				$data['title']    = 'Emp Documents'; 
				$data['module']   = "employee";
				$data['page']     = "view_emp_document_expiry";   
				$data['expirydata']      = $this->Employees_model->Expirydata_redzone();
				$data['expirydata_orengezone']      = $this->Employees_model->Expirydata_orengezone();
				$data['expirydata_greenzone']      = $this->Employees_model->Expirydata_greenzone();
